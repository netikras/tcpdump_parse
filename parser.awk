...
function show(id, time, sender, duration, recipient) {
    nicetime = strftime(timeFormat, time);
    printf "%s %s %s %.8f %s\n", id, nicetime, sender, duration, recipient;
}
...
}
{
 ...
 if (client != "" && request[client]status == "s") {
    ...
    show(request[client, "id"], request[client, "sendEndTime"], client, r, request[client, "server"]);
...
END {
	...
	for (client in request) {
		if (request[client] == "r") {
			r = request[client, "receiveStartTime"] - request[client, "sendEndTime"];
            show(request[client, "id"], strftime(timeFormat, request[client, "sendEndTime"]), client, r, request[client, "server"]);
